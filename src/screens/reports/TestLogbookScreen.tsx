...
export const TestLogbookScreen = () => {
	...
	useFocusEffect(
		useCallback(() => {
			const loadFlights = async () => {
				try {
					const result = await db.getAllAsync('SELECT * FROM flights WHERE type = ? AND pdf_path IS NOT NULL ORDER BY date DESC', ['Ensayo']);
	...
	const exportZIP = async () => {
		try {
			...
			const outDir = new FileSystem.Directory(FileSystem.Paths.document, 'DFM', 'Export', 'BitacoraEnsayo');
			try { outDir.create({ intermediates: true, idempotent: true }); } catch {}
			const outFile = new FileSystem.File(outDir, 'bitacora_ensayo.zip');
	...
	return (
		<ScreenLayout>
			<View style={styles.header}>
				<Text style={styles.title}>Bitácora Ensayo</Text>
